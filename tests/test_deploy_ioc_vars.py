...
DEPLOY_IOC_VARS_FILES = [os.path.splitext(f)[0] for f in os.listdir("roles/deploy_ioc/vars") if f.endswith(".yml")]

pytestmark = pytest.mark.parametrize("deploy_ioc_var_file", DEPLOY_IOC_VARS_FILES, indirect=True)
...
def test_deploy_ioc_var_file_required_module_exists(deploy_ioc_var_file):
    if "deploy_ioc_required_module" in deploy_ioc_var_file.data:
        if deploy_ioc_var_file.data["deploy_ioc_required_module"]:
            assert os.path.exists(os.path.join("roles/install_module/vars", f"{deploy_ioc_var_file.data["deploy_ioc_required_module"]}.yml"))